...
Plugin do Claude Code para scraping de catálogos JEM. **Planos 1, 2, 4, 3a, 5 e 6 completos, na `main`, pushados. 187 testes passando (+1 skip: smoke Playwright).** Falta só o **Plano 7 — empacotamento** (o que transforma tudo isso num plugin instalável). (Plano 3b — auth/token — deferido; ver §5.1/§5.2.)
...
7. **Empacotamento do plugin (ÚLTIMO — próximo)** — `plugin.json`, as skills markdown (`scrape-onboarding`, `scrape-product-catalog` orquestrador, `scrape-compliance-gate`, `scrape-warmup`, `scrape-normalize-export`, `scrape-run-plan`), commands (`/scrape-setup`, `/scrape-init`, `/scrape-status`), references, o agent `scrape-run-auditor`, hooks.json. **Decisão do usuário: empacotar por último, depois das fases.** Nota: quase tudo é markdown/JSON (pouca superfície de TDD) → usar o toolchain `plugin-dev` (agents `plugin-validator`/`skill-reviewer`), não o SDD-TDD. Ao fiar o executor do `deploy_actions`, manter o segredo estritamente no stdin do subprocess (ler `stdin_file`), nunca no argv.
